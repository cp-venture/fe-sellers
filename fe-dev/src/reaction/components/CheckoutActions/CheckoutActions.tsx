...
class CheckoutActions extends Component {
  ...
  get shippingMethod() {
    ...
    const { selectedFulfillmentOption } = cart.checkout.fulfillmentGroups[0];
  ...
  setShippingMethod = async (shippingMethod) => {
    ...
    const fulfillmentOption = {
      fulfillmentGroupId: cart.checkout.fulfillmentGroups[0]._id,
  ...
  render() {
    ...
    const {fulfillmentGroups} = this.props.cart.checkout;
    const [fulfillmentGroup] = this.props.cart.checkout.fulfillmentGroups;
    const {items} = cart;
